...
def data_quality_checks(con):
    ...
    if null_prods > 0:
        print(f"   ⚠️ WARNING: {null_prods} sales with unknown Product IDs!")
    else:
        print("   ✓ Grocery product joins look good.")

    # Check 2: Null municipality in sales
    null_mun = con.execute("SELECT COUNT(*) FROM silver_grocery_sales WHERE municipality IS NULL").fetchone()[0]
    if null_mun > 0:
    ...
    pop_years = con.execute("SELECT MIN(year), MAX(year) FROM silver_population_total").fetchone()
    print(f"   ✓ Population data covers years: {pop_years[0]} - {pop_years[1]}")

    # Check 4: Municipalities in population vs sales
    pop_muns = con.execute("SELECT COUNT(DISTINCT municipality_name) FROM silver_population_total").fetchone()[0]
    sales_muns = con.execute("SELECT COUNT(DISTINCT municipality) FROM silver_grocery_sales").fetchone()[0]
    print(f"   ✓ Municipalities - Population: {pop_muns}, Sales: {sales_muns}")

    # Check 5: Tourism data coverage
    tourism_years = con.execute("SELECT MIN(year), MAX(year) FROM silver_tourism").fetchone()
