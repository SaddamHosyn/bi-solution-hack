...
def get_connection():
    ...
    return duckdb.connect(
        str(project_root / "warehouse" / "warehouse.duckdb"), read_only=True
    )
...
def render_custom_metric(label, value, prefix="", suffix="", prev_value=None):
    ...
    if yoy is not None:
        ...
        delta_html = (
            "<span style='color: #9e9e9e; font-size: 0.8rem;'>No prior data</span>"
        )
...
municipalities = (
    con.execute(
        "SELECT DISTINCT municipality FROM gold_sales_by_municipality ORDER BY municipality"
    )
    .df()["municipality"]
    .tolist()
)
selected_municipalities = st.sidebar.multiselect(
    "🏘️ Region", municipalities, default=municipalities
)
mun_filter_sql = (
    ", ".join([f"'{m}'" for m in selected_municipalities])
...
st.sidebar.markdown("---")
st.sidebar.info(
    f"Showing data for **{len(selected_municipalities)}** regions in **{selected_year}**."
)
...
# Data Queries
current_revenue = get_metric_value(
    f"SELECT SUM(total_revenue) FROM gold_sales_by_municipality WHERE year={selected_year} AND municipality IN ({mun_filter_sql})"
)
current_transactions = get_metric_value(
    f"SELECT SUM(total_transactions) FROM gold_sales_by_municipality WHERE year={selected_year} AND municipality IN ({mun_filter_sql})"
)
current_population = get_metric_value(
    f"SELECT SUM(population) FROM gold_sales_by_municipality WHERE year={selected_year} AND municipality IN ({mun_filter_sql})"
)
top_mun = con.execute(
    f"SELECT municipality FROM gold_sales_by_municipality WHERE year={selected_year} AND municipality IN ({mun_filter_sql}) ORDER BY revenue_per_capita DESC LIMIT 1"
).fetchone()

# Previous Year Data
if previous_year:
    prev_revenue = get_metric_value(
        f"SELECT SUM(total_revenue) FROM gold_sales_by_municipality WHERE year={previous_year} AND municipality IN ({mun_filter_sql})"
    )
    prev_transactions = get_metric_value(
        f"SELECT SUM(total_transactions) FROM gold_sales_by_municipality WHERE year={previous_year} AND municipality IN ({mun_filter_sql})"
    )
    prev_population = get_metric_value(
        f"SELECT SUM(population) FROM gold_sales_by_municipality WHERE year={previous_year} AND municipality IN ({mun_filter_sql})"
    )
else:
...
with col1:
    render_custom_metric(
        "💰 Total Revenue (EUR)", current_revenue, "€", "", prev_revenue
    )

with col2:
    render_custom_metric(
        "🧾 Transactions", current_transactions, "", " transactions", prev_transactions
    )

with col3:
    render_custom_metric(
        "👥 Population", current_population, "", " people", prev_population
    )
...
with c1:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                size=8, color="white", line=dict(width=2, color=COLORS["primary"])
            ),
        )
    )
...
with c2:
    scatter_data = (
        con.execute(
            f"SELECT * FROM gold_sales_by_municipality WHERE year={selected_year} AND municipality IN ({mun_filter_sql})"
        )
        .df()
    ...
    if not scatter_data.empty:
        ...
        if event and event.selection["points"]:
            ...
            clicked_muns = (
                scatter_data.iloc[selected_indices]["municipality"].unique().tolist()
            )
            if clicked_muns:
...
with c3:
    cat_data = con.execute(
        f"SELECT category, total_revenue FROM gold_category_performance WHERE year={selected_year} ORDER BY total_revenue DESC"
    ).df()
    fig = px.pie(
        cat_data,
...
with c4:
    monthly = con.execute(
        f"SELECT month, SUM(total_revenue) as rev FROM gold_monthly_sales WHERE year={selected_year} GROUP BY month ORDER BY month"
    ).df()
    ...
    fig = px.bar(
        ...
        color="rev",
        color_continuous_scale=[(0, "#e0f2f1"), (1, COLORS["primary"])],
    )
...
with c5:
    tourism_data = con.execute(
        f"SELECT month, total_revenue, total_visitors FROM gold_tourism_sales WHERE year = {selected_year} ORDER BY month"
    ).df()
...
with c6:
    ...
    if not ranking_data.empty:
        ...
        ranking_data["color"] = ranking_data["revenue_per_capita"].apply(
            lambda x: COLORS["primary"] if x >= avg_rpc else COLORS["alert"]
        )

        fig = go.Figure(
            go.Bar(
                x=ranking_data["revenue_per_capita"],
                y=ranking_data["municipality"],
                orientation="h",
                marker_color=ranking_data["color"],
            )
        )
        fig.add_vline(
            x=avg_rpc,
            ...
            annotation_text=f"Avg: €{avg_rpc:,.0f}",
        )
        fig.update_layout(
            title="Revenue Per Capita Ranking",
...
with c_export:
    ...
    st.download_button(
        ...
        f"grocery_report_{selected_year}.csv",
        "text/csv",
    )

    full_data = con.execute(
        f"SELECT * FROM gold_sales_by_municipality WHERE municipality IN ({mun_filter_sql})"
    ).df()
    st.download_button(
        "📊 Full History",
        full_data.to_csv(index=False),
        "grocery_full_history.csv",
        "text/csv",
    )
