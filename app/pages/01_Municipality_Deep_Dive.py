...
prev_data = (
    df[df["year"] == (latest_year - 1)].iloc[0]
    if (latest_year - 1) in df["year"].values
    else None
)
...
with col2:
    delta = calc_delta(
        ...
        prev_data["total_revenue"] if prev_data is not None else None,
    )
    st.metric("💰 Revenue", f"€{latest_data['total_revenue']:,.0f}", delta)
with col3:
    delta = calc_delta(
        latest_data["population"],
        prev_data["population"] if prev_data is not None else None,
    )
